...
Connection.prototype.writeEvents = function(streamId, expectedVersion, requireMaster, events, credentials, callback) {
    ...
    for(var i=0; i<events.length; i++) {
        ...
        if (events[i].metadata) {            
            events[i].metadataContentType = 1;
            var json = JSON.stringify(events[i].metadata);
            events[i].metadata = new Buffer(json);
        }
    }
...
function unpackEventRecord(eventRecord) {
    ...
    event.metadata = JSON.parse(eventRecord.metadata.toBuffer());
